...
参C++值语义(ValueSemantics)与对象语义(Reference semantics)。
...
| 深拷贝 | 浅拷贝 |
| 深赋值 | 浅赋值 |
| Equality-Based 判等 | Identity-Based 判等 |

!["C++ Terminology"](/res/image/object.svg)

1. 值变量
值变量就直接指向了object自身，也就是一个精确的内存地址。
值变量和object是精确的1：1关系。
值变量和object的生命周期是等同的。
2. 引用变量
引用变量可能指向一个类型兼容的object
引用变量可以有多个指向一个object
引用变量和object的对象生命周期是独立的。
3. 值成员
值成员和所属的object是紧耦合关系，在内存上是紧密排列的，值成员的生命周期与所属的object是相同的。
4. 引用成员
引用成员和所属的object在内存分布上分离的，并且在生命周期上也是独立的。
5. 深拷贝
产生一个独立的object，不仅在内存上原来的值和拷贝的值是独立的，
而且在生命周期上也是独立的。
6. 浅拷贝
仅仅产生一个引用，在内存上原来的引用和新的引用是指向同样的内存空间，虽然引用变量的生命周期是独立的，但是它们所引用的那个object的生命周期是关联的，不存在独立性。
7. 深赋值
深赋值会将赋值的目标object变成与原object完全一致。
表现为：（1）会递归地对成员使用拷贝赋值运算符。（2）会递归地使用移动赋值运算来转移所有权。
8. 浅赋值
仅仅改变应用的指向而已。
9. 值判等
会递归地对所有成员进行判断，只有所有的成员相等，才被认为相同。
10. 引用判等
仅仅判断它们指向的内存地址是否相同。
